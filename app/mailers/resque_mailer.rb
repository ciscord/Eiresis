class ResqueMailer < ActionMailer::Base
  ...
  def notification(alert_id)
    ...
    if to_id
      mail(to: "discussion+#{to_id}@eiresis.com", bcc: @user.email, subject: subject, template_name: template_name) # TODO: extract email
    else
  ...
  def massive_email(from_id, to_ids, group_id, subject, body)
    ...
    mail(bcc: @to.map(&:email), from: ENV['NOREPLY_EMAIL'], reply_to: @from.email, to: 'test@eiresis.com', subject: subject) # TODO: extract email
  end
