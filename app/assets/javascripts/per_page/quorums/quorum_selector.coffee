class eiresis.QuorumSelector
  constructor: ->
    ...
    quorum_.on 'change', (e) ->
      ...
      if timefixed_
        $.ajax
          url: eiresis.paths.quorums.dates
          data:
            id: id_
            group_id: eiresis.groupId
          success: ->
            choose_vote.show()
            now_ = new Date
            end_ = addMinutes(now_, parseInt(minutes_) + eiresis.debateVoteDifference)
            #votation start time
            ...
            choose_vote.find('.start_vot').html eiresis.i18n.proposals.debate.end + ' (' + dateToString(upperend_) + ')'
