window.ProposalsShow =
  ...
  firstCheck: false
  init: ->
    @currentView = if eiresis.signed_in then 1 else 3
    ...
    $(document).on 'ajax:beforeSend', '.icon-sad-change', (n, xhr)->
      ...
      $(this).parent().find('.vote_comment').hide()
      $(this).parent().find('.loading').show()
      $(".reply_textarea[data-id=#{num}]").focus().attr('placeholder', eiresis.i18n.rankdown_reason).effect('highlight', {}, 3000)
    $(document).ajaxError (e, XHR, options)->
      if XHR.status == 401
        window.location.replace(eiresis.new_user_session_path)
    if @voting
      new eiresis.ProposalVotationManager()
    else
      @currentPage++;
      $.ajax
        url: @contributesUrl,
        ...
        data: {
          ...
          comment_id: eiresis.show_comment_id #show contribute if requested
        },
        type: 'get',
        complete: ->
    ...
    if eiresis.show_section_id
      $(".contribute-button[data-section_id=#{eiresis.show_section_id}]").trigger('click', [eiresis.show_comment_id])
    ...
    $('[data-contribute-area]').each ->
      if $(this).attr('data-initialized') != 1
        ...
        $(this).charCount
          ...
          warning: 100,
          counterText: eiresis.i18n.proposals.charactersLeft
  ...
  contribute: (section_id)->
    ...
    eiresis.viewport.bind "scroll mousedown DOMMouseScroll mousewheel keyup", (e)->
      if matchMedia(Foundation.media_queries['medium']).matches && e.which > 0 || e.type is "mousedown" || e.type is "mousewheel"
        eiresis.viewport.stop().unbind 'scroll mousedown DOMMouseScroll mousewheel keyup'
    return false
  edit_contribute: (id)->
    close_all_dropdown()
    $.ajax
  ...
  open_right_contributes: (_this, comment_id)->
    ...
    $('.text', _this).text(eiresis.i18n.proposals.closeContributes)
    hideLeftPanel()
  ...
  close_right_contributes: (_this)->
    ...
    $('.text',
      _this).text(eiresis.i18n.proposals.showGiveContributes).append(" (#{_this.attr('data-unread_contributes_num')}/#{_this.attr('data-contributes_num')})")
...
#2 - fetched - closed
  init_contributes_button: ->
    $(".contribute-button").click (event, comment_id)->
      ...
      $(".contribute-button").each ->
        ...
        if this_[0] != this     #for each right panel that is not the opened one
          if his_status is '1'
            ...
            $('.text',
              this).text(eiresis.i18n.proposals.showGiveContributes).append(" (" + $(this).attr('data-unread_contributes_num') + "/" + $(this).attr('data-contributes_num') + ")")
      ...
      if this_status is '0'  #closed and never fetched
        ...
        $('.text', this).text(eiresis.i18n.proposals.closeContributes)
        hideLeftPanel()
        fetched = $('<div data-section_id="' + section_id + '"class="suggestion_right"></div>')
        fetched.append('<div style="margin:auto;text-align:center;">' + eiresis.loadingImageTag + '<br/><b>' + eiresis.i18n.proposals.loadingContributes + '</b></div>')
        ...
        $.ajax
          ...
          data:
            ...
            disable_limit: true
            view: if eiresis.signed_in then 1 else 3
          ,
          type: 'get',
          dataType: 'script',
          complete: ->
          ...
          error: (xhr, ajaxOptions, thrownError)->
            $(".loading", fetched).hide()
            if xhr.status == '404'
              $(fetched).html(eiresis.i18n.proposals.errorLoadingParagraph)
        $('.suggestion_right').bind 'mousewheel DOMMouseScroll', (e)->
          if matchMedia(Foundation.media_queries['medium']).matches
            eiresis.scrollLock(this,e)
  ...
  init_countdowns: ->
    ...
    $('.date-creation').each ->
      ...
      ,
        $.countdown.regionalOptions[eiresis.i18n.locale]
    ...
    $('.date-update').countdown $.extend
      ...
      significant: 1
      format: 'YODHMS'
      layout: eiresis.i18n.countdown.layout2
    ,
      $.countdown.regionalOptions[eiresis.i18n.locale]
