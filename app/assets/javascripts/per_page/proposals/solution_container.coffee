class eiresis.SolutionContainer
  ...
  constructor: (@id)->
    if @id instanceof jQuery
      ...
      @element = $(eiresis.SolutionContainer.selector).filter("[data-solution_id='#{@id}']")
    ...
    @sections = @element.find(eiresis.SectionContainer.selector)
  persisted: ->
    @element.data('persisted')
  moveUp: ->
    to_exchange = @element.prevAll(eiresis.SolutionContainer.selector).first()
    @element.after to_exchange
    ProposalsEdit.updateSolutionSequences()
  moveDown: ->
    to_exchange = @element.nextAll(eiresis.SolutionContainer.selector).first()
    @element.before to_exchange
    ProposalsEdit.updateSolutionSequences()
  remove: ->
    if confirm eiresis.i18n.proposals.edit.removeSolutionConfirm
      if @persisted
        @destroyField.val(1)
        @element.fadeOut()
      else
        @element.fadeOut ->
          @element.remove()
      @element.nextUntil(null, eiresis.SolutionContainer.selector).each ->
