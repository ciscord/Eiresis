window.ProposalsEdit =
  ...
  init: ->
    ...
    start_field.fdatetimepicker($.fn.fdatetimepicker.dates[eiresis.i18n.locale]).on 'hide', (event) ->
      ...
      return
    end_field.fdatetimepicker $.fn.fdatetimepicker.dates[eiresis.i18n.locale]
    ...
    $(document).on 'click', '[data-remove-solution]', =>
      ...
      return false
    #editors
    $(eiresis.SectionContainer.selector).each ->
      container = new eiresis.SectionContainer(@)
    ...
    suggestion_right_.bind 'mousewheel DOMMouseScroll', (e)->
      if (matchMedia(Foundation.media_queries['medium']).matches)
        eiresis.scrollLock(suggestion_right_, e)
    ...
    #navigator
    @navigator = new eiresis.ProposalNavigator
  ...
  fillCleanFields: ->
    ...
    if ProposalsEdit.contributesCount > 0
      if integrated_ == ''
        if !confirm(eiresis.i18n.proposals.edit.updateConfirm)
          return false
    try
      id = undefined
  ...
  toggleSolutions: (compress) ->
    $('.solution_main').each ->
      solution = new eiresis.SolutionContainer($(@))
  ...
  updateSequences: ->
    $('.sections_column, .solutions_column').each ->
      i = 0
      $(this).find(eiresis.SectionContainer.selector).each (el) ->
        section = new eiresis.SectionContainer($(this))
        section.setSeq(i++)
  updateSolutionSequences: ->
    i = 0
    $('.solution_main').each ->
      ...
      solution.setSeq(i++)
  addSection: ->
    title = eiresis.i18n.proposals.edit.paragraph + ' ' + (ProposalsEdit.sectionsCount + 1)
    sectionId = ProposalsEdit.sectionsCount
    section = $(Mustache.to_html($('#section_template').html(), section:
      id: sectionId
      seq: sectionId + 1
      removeSection: eiresis.i18n.proposals.edit.removeSection
      title: title
      paragraphId: ''
      content: ''
      contentDirty: ''
    ...
    section.fadeIn()
    new (eiresis.SectionContainer)(section).initCkEditor()
  ...
  addSolutionSection: (solutionId)->
    ...
    title = eiresis.i18n.proposals.edit.paragraph + ' ' + (sectionId + 1)
    ...
    new eiresis.SectionContainer(solutionSection).initCkEditor()
  ...
  addSolution: ->
    ...
    options = solution:
      ...
      persisted: true
      title_placeholder: eiresis.i18n.proposals.edit.titlePlaceholder
      solution_title: eiresis.i18n.proposals.edit.solutionTitle
      title: ''
      removeSolution: eiresis.i18n.proposals.edit.removeSolution
      addParagraph: eiresis.i18n.proposals.edit.addParagraph
    ...
    solution.find(eiresis.SectionContainer.selector).each (idx, section)->
      new eiresis.SectionContainer(section).initCkEditor()
