class eiresis.ProposalNavigator
  constructor: ->
    ...
    @solution_navigators.on 'click', (event) ->
      if this is event.target
        ...
        solution = new eiresis.SolutionContainer($(this).data('solution_id'))
  ...
  getSectionActionSubject: (list_element)->
    ...
    new eiresis.SectionContainer(section_id)
  getSolutionActionSubject: (list_element)->
    solution_id = list_element.data('solution_id')
    new eiresis.SolutionContainer(solution_id)
  ...
  removeSolution: (solutionId)->
    toRemove = new eiresis.SolutionContainer(solutionId)
    if toRemove.remove()
  ...
  addSolutionNavigator: (solutionId)->
    sections = []
    solution = new eiresis.SolutionContainer(solutionId)
    for section in solution.sections
      sectionContainer = new eiresis.SectionContainer(section)
