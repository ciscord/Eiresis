window.GroupsIndex =
  ...
  checkCharacters: ->
    if $('#search').val().length > 2 or $('#interest_border').val() != ''
      ...
      alert eiresis.i18n.groups.queryOrPlace
      false
  initSwitchButtons: ->
    $('#area').switchButton
      ...
      on_title: eiresis.i18n.groups.exactBorder
      off_title: eiresis.i18n.groups.areaBorder
    $('#and').switchButton
      checked: GroupsIndex.andParam
      on_label: $('<div class="switch_opt">&</div>')
      off_label: $('<div class="switch_opt">||</div>')
      on_title: eiresis.i18n.groups.allWords
      off_title: eiresis.i18n.groups.oneWord
