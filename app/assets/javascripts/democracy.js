...
function scrollToElement(element, callback) {
    ...
    eiresis.viewport.bind("scroll mousedown DOMMouseScroll mousewheel keyup", function (e) {
        if (e.which > 0 || e.type === "mousedown" || e.type === "mousewheel") {
            eiresis.viewport.stop().unbind('scroll mousedown DOMMouseScroll mousewheel keyup'); // This identifies the scroll as a user action, stops the animation, then unbinds the event straight after (optional)
        }
    });
    return false;
}
    ...
    start.fdatetimepicker()
        .on('hide', function (event) {
            ...
            if (eventEndTime_ < minStartTime) {
                ...
                showOnField(end, eiresis.i18n.datepicker.changed);
            }
...
function disegnaCountdown() {
    $('div[data-countdown]').each(function () {
        $(this).countdown($.extend({
            ...
            significant: 1,
            format: 'ms',
            layout: eiresis.i18n.countdown.layout1
...
function poll() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            read_container.append($('<a href="#" onclick="sign_all_as_read(' + data.id + ');return false;">' + eiresis.i18n.alerts_sign_has_read + '</a>'));
            read_container.append(' · ');
            var url = '/users/alarm_preferences';
            if (eiresis.i18n.l !== '')
                url += '?l='+eiresis.i18n.l;
            read_container.append($('<a href="'+url+'">' + eiresis.i18n.alarm_settings + '</a>'));
            ...
            for (var j = 0; j < data.alerts.length; j++) {
                ...
                if (!alert.checked) {
                    alert_container.append($('<i class="fa fa-circle-o read" title="' + eiresis.i18n.alert_read + '" data-type_id="' + data.id + '" onclick="read_notifica(this);return false;"></i>'));
                }
                sub_container.append(li_alert_con);
            }
            if (data.alerts.length == 0) {
                sub_container.append($('<span>' + eiresis.i18n.no_alerts + '</span>'));
            }
...
function eiresis_close_reveal() {
    $('.reveal-modal:visible').foundation('reveal', 'close');
}

function eiresis_reveal(element_, remove_on_close) {
    ...
    eiresis_reveal_refresh(element_);
}

//to call when reveal content is replaced by a new one
function eiresis_reveal_refresh(element_) {
